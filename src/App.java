...
public class App {
    public static void main(String[] args) throws Exception {
        ...
        try (InputStream is = new FileInputStream(propsFile)) {
            ...
            // logger.fatal("Error while loading the 'config.properties' file, aborting
            // program.",e);
            System.exit(1);
        }
        ...
        for (Entry<Object, Object> property : props.entrySet()) {
            ArticleMatchers
                    .add(new ArticleSourceProperties(property.getKey().toString(), property.getValue().toString()));
        }
        ...
        for (ArticleSourceProperties articleProps : ArticleMatchers) {
            ...
            for (Element articleScraped : articlesScraped) {
                Article ar = new Article(articleScraped.select(articleProps.getUrlMatcher()).first().attr("abs:href"),
                        articleScraped.select(articleProps.getTitleMatcher()).first().text());
